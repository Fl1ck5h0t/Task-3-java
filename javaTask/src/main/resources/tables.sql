...
CREATE TABLE progresses(
    id int8 primary key unique not null,
    playerId int8 references players(playerId) on delete cascade ,
    resourceId int8,
    score int4,
    maxScore int4,
...
CREATE TABLE player_currency_map(
    playerId int8 references players(playerId) on delete cascade ,
    currencyId int8 references currencies(id) on delete cascade
);

CREATE TABLE player_item_map(
    playerId int8 references players(playerId) on delete cascade ,
    itemId int8 references items(id) on delete cascade
)
